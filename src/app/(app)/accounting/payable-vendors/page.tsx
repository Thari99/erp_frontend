...
import { api, ApiError, type PayableVendor } from '@/lib/api';
import { useRequireLogin } from '@/lib/use-require-login';

export default function ManagePayableVendorsPage() {
  ...
  const load = useCallback(async () => {
    try {
      setVendors(await api.listPayableVendors());
    } catch (err) {
      if (!requireLogin(err)) setError(err instanceof ApiError ? err.message : 'Failed to load vendors');
    } finally {
      setIsLoading(false);
    }
  }, [requireLogin]);
  ...
  function startEdit(vendor: PayableVendor) {
  ...
  async function submitEdit(event: React.FormEvent) {
    ...
    setError(null);
    try {
      await api.updatePayableVendor(editingId, {
        name: editName.trim(),
      ...
      if (!requireLogin(err)) setError(err instanceof ApiError ? err.message : 'Failed to update vendor');
    } finally {
      setBusyId(null);
    }
  }

  async function toggleActive(vendor: PayableVendor) {
    setBusyId(vendor.id);
    setError(null);
    try {
      await api.updatePayableVendor(vendor.id, { isActive: !vendor.isActive });
      await load();
    } catch (err) {
      if (!requireLogin(err)) setError(err instanceof ApiError ? err.message : 'Failed to update vendor');
    } finally {
  ...
  async function handleDelete(vendor: PayableVendor) {
    if (!window.confirm(`Delete "${vendor.name}"?`)) return;
    setBusyId(vendor.id);
    setError(null);
    try {
      await api.deletePayableVendor(vendor.id);
      await load();
    } catch (err) {
      if (!requireLogin(err)) setError(err instanceof ApiError ? err.message : 'Failed to delete vendor');
    } finally {
      setBusyId(null);
    }
  }
    ...
    <main className="px-6 py-10 text-slate-50">
      <div className="mx-auto max-w-4xl">
        <header className="mb-8 flex flex-wrap items-start justify-between gap-3">
          ...
          <Link
            href="/accounting/payable-vendors/new"
          ...
          <table className="w-full text-left text-sm">
            <thead>
              <tr className="bg-slate-900 text-left text-xs uppercase tracking-wide text-slate-500">
                ...
                <th className="px-4 py-3 font-medium">Balance Owed</th>
                    ...
                    {editingId === vendor.id ? (
                      ...
                      <>
                        ...
                        <td className="px-4 py-3">
                          <span className={Number(vendor.balance) > 0 ? 'text-amber-400' : 'text-slate-400'}>
                            Rs. {Number(vendor.balance).toFixed(2)}
                          </span>
                        </td>
                        <td className="px-4 py-3">
                          <span
