...
import { api, ApiError, type BarCategory } from '@/lib/api';
...
export default function ManageCategoryPage() {
  ...
  const load = useCallback(async () => {
    try {
      setCategories(await api.listBarCategories());
    } catch (err) {
  ...
  function startEdit(category: BarCategory) {
  ...
  async function submitEdit(event: React.FormEvent) {
    ...
    setError(null);
    try {
      await api.updateBarCategory(editingId, { name: editName.trim() });
      setEditingId(null);
      await load();
    } catch (err) {
  ...
  async function toggleActive(category: BarCategory) {
    setBusyId(category.id);
    setError(null);
    try {
      await api.updateBarCategory(category.id, { isActive: !category.isActive });
      await load();
    } catch (err) {
      if (!requireLogin(err)) setError(err instanceof ApiError ? err.message : 'Failed to update category');
    } finally {
  ...
  async function handleDelete(category: BarCategory) {
    if (!window.confirm(`Delete "${category.name}"?`)) return;
    setBusyId(category.id);
    setError(null);
    try {
      await api.deleteBarCategory(category.id);
      await load();
    } catch (err) {
            ...
            <tbody>
              {isLoading ? (
                ...
                <tr>
                  <td colSpan={3} className="px-4 py-6 text-center text-slate-500">
                    No categories yet —{' '}
                    <Link href="/bar/categories/new" className="text-emerald-400 underline">
                      create one
                    </Link>
                    .
                  </td>
