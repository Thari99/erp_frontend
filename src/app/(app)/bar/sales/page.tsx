...
export default function BarSalesPage() {
  ...
  const load = useCallback(async () => {
    try {
      setSales(await api.listBarSales());
    } catch (err) {
  ...
  async function handleVoid(sale: BarSale) {
    if (!window.confirm(`Void ${sale.saleReference ?? `#${sale.saleNo}`}? Stock will be restored and the payment reversed.`)) return;
    setVoidingId(sale.id);
    setError(null);
    try {
      await api.voidBarSale(sale.id);
      await load();
    } catch (err) {
                  ...
                  <Fragment key={sale.id}>
                    <tr className="border-t border-slate-800 text-slate-200 hover:bg-slate-800/60">
                      <td className="px-4 py-3 font-mono text-xs text-slate-400">
                        {sale.saleReference ?? `BAR-${String(sale.saleNo).padStart(4, '0')}`}
                      </td>
                        ...
                        >
                          <span
                            className={`h-1.5 w-1.5 flex-none rounded-full ${
                              sale.voidedAt ? 'bg-red-500' : sale.status === 'HELD' ? 'bg-amber-500' : 'bg-emerald-500'
                            }`}
                          />
                      ...
                      <td className="px-4 py-3">
                        <div className="flex flex-wrap items-center gap-1">
                          ...
                          {sale.status === 'COMPLETED' && (
                            <Link
                              href={`/bar/sales/${sale.id}?autoprint=1`}
                              className="rounded border border-slate-600 px-2 py-1 text-xs text-slate-300 hover:border-slate-500"
                            >
                              Print
                            </Link>
                          ...
                          <ul className="space-y-1 text-xs text-slate-400">
                            {sale.items.map((item) => (
                              <li key={item.id} className="flex justify-between">
                                <span>
                                  {item.quantity} × {item.product.name} @ Rs. {Number(item.unitPrice).toFixed(2)}
                                </span>
