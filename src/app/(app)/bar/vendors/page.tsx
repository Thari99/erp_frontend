...
import { api, ApiError, type BarVendor } from '@/lib/api';
...
export default function ManageSuppliersPage() {
  ...
  const load = useCallback(async () => {
    try {
      setVendors(await api.listBarVendors());
    } catch (err) {
  ...
  function startEdit(vendor: BarVendor) {
  ...
  async function submitEdit(event: React.FormEvent) {
    ...
    setError(null);
    try {
      await api.updateBarVendor(editingId, {
        name: editName.trim(),
  ...
  async function toggleActive(vendor: BarVendor) {
    setBusyId(vendor.id);
    setError(null);
    try {
      await api.updateBarVendor(vendor.id, { isActive: !vendor.isActive });
      await load();
    } catch (err) {
      if (!requireLogin(err)) setError(err instanceof ApiError ? err.message : 'Failed to update supplier');
    } finally {
  ...
  async function handleDelete(vendor: BarVendor) {
    if (!window.confirm(`Delete "${vendor.name}"?`)) return;
    setBusyId(vendor.id);
    setError(null);
    try {
      await api.deleteBarVendor(vendor.id);
      await load();
    } catch (err) {
    ...
    <main className="px-6 py-10 text-slate-50">
      <div className="mx-auto max-w-4xl">
        <header className="mb-8 flex flex-wrap items-start justify-between gap-3">
          <div>
            <p className="text-sm text-emerald-400">Bar · Suppliers</p>
            <h1 className="text-2xl font-semibold">Manage Suppliers</h1>
          </div>
          <Link
            href="/bar/vendors/new"
            ...
            <tbody>
              {isLoading ? (
                ...
                <tr>
                  <td colSpan={5} className="px-4 py-6 text-center text-slate-500">
                    No suppliers yet —{' '}
                    <Link href="/bar/vendors/new" className="text-emerald-400 underline">
                      create one
                    </Link>
                    .
                  </td>
