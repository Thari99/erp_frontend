...
import { api, ApiError, type BarPurchaseOrder } from '@/lib/api';
...
const STATUS_DOT: Record<BarPurchaseOrder['status'], string> = {
  OPEN: 'bg-amber-500',
...
export default function ManagePurchaseOrdersPage() {
  ...
  const [orders, setOrders] = useState<BarPurchaseOrder[]>([]);
  ...
  function toggleMenu(order: BarPurchaseOrder, event: React.MouseEvent<HTMLButtonElement>) {
  ...
  const load = useCallback(async () => {
    try {
      setOrders(await api.listBarPurchaseOrders());
    } catch (err) {
  ...
  function refLabel(order: BarPurchaseOrder) {
    return order.poReference ?? `PO-${order.poNo}`;
  }

  async function handleCancel(order: BarPurchaseOrder) {
    setOpenMenuId(null);
    if (!window.confirm(`Cancel ${refLabel(order)}?`)) return;
    setBusyId(order.id);
    setError(null);
    try {
      await api.cancelBarPurchaseOrder(order.id);
      await load();
    } catch (err) {
  ...
  async function handleDelete(order: BarPurchaseOrder) {
    setOpenMenuId(null);
    if (!window.confirm(`Delete ${refLabel(order)}? This can't be undone.`)) return;
    setBusyId(order.id);
    setError(null);
    try {
      await api.deleteBarPurchaseOrder(order.id);
      await load();
    } catch (err) {
    ...
    <main className="px-6 py-10 text-slate-50">
      <div className="mx-auto max-w-4xl">
        <header className="mb-8 flex flex-wrap items-start justify-between gap-3">
          <div>
            <p className="text-sm text-emerald-400">Bar · Purchase Orders</p>
            <h1 className="text-2xl font-semibold">Manage Purchase Orders</h1>
            <p className="mt-1 text-sm text-slate-500">
              Manage suppliers under <Link href="/bar/vendors" className="text-emerald-400 underline">Suppliers</Link>.
            </p>
          </div>
          <Link
            href="/bar/purchase-orders/new"
                        ...
                        <div className="inline-block text-left" ref={openMenuId === order.id ? menuRef : undefined}>
                          ...
                          {openMenuId === order.id && menuPosition && (
                            ...
                            >
                              {order.status === 'OPEN' && (
                                <Link
                                  href={`/bar/purchase-orders/${order.id}/edit`}
                              ...
                              <Link
                                href={`/bar/purchase-orders/${order.id}`}
                                onClick={() => setOpenMenuId(null)}
                                className="block px-3 py-1.5 text-xs text-slate-200 hover:bg-slate-700"
                              >
                                Read more
                              ...
                              {order.status !== 'CANCELLED' && (
                                <Link
                                  href={`/bar/gate-pass?po=${order.id}`}
                                  onClick={() => setOpenMenuId(null)}
                                  className="block px-3 py-1.5 text-xs text-slate-200 hover:bg-slate-700"
                                >
                                  Gate Pass
                              ...
                              <Link
                                href={`/bar/purchase-orders/${order.id}?autoprint=1`}
                                onClick={() => setOpenMenuId(null)}
                                className="block px-3 py-1.5 text-xs text-slate-200 hover:bg-slate-700"
                              >
                                Print
                          ...
                          <ul className="space-y-1 text-xs text-slate-400">
                            {order.items.map((item) => (
                              <li key={item.id} className="flex justify-between">
                                <span>
                                  {item.orderedQty} × {item.product.name} @ Rs. {Number(item.unitCost).toFixed(2)}
                                </span>
