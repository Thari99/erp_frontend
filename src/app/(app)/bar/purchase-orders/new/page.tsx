...
import { api, ApiError, type BarProduct, type BarPurchaseOrder, type BarVendor } from '@/lib/api';
...
export default function CreatePurchaseOrderPage() {
  ...
  const [createdPo, setCreatedPo] = useState<BarPurchaseOrder | null>(null);

  // Informational only — the PO date shown here is "now"; the actual createdAt is set by
  // the server at save time and may differ by a few seconds.
  const [poDatePreview] = useState(() => formatDateTime(new Date()));
  ...
  const load = useCallback(async () => {
    try {
      const [supplierList, productList] = await Promise.all([api.listBarVendors(), api.listBarProducts()]);
  ...
  async function handleAddSupplier(event: React.FormEvent) {
    ...
    try {
      const supplier = await api.createBarVendor({ name: newSupplierName, phone: newSupplierPhone || undefined });
  ...
  async function handleCreate(event: React.FormEvent) {
    ...
    setIsSaving(true);
    try {
      const po = await api.createBarPurchaseOrder({
        vendorId: supplierId,
      ...
      if (!requireLogin(err)) setError(err instanceof ApiError ? err.message : 'Failed to create purchase order');
    } finally {
      setIsSaving(false);
    }
  }
      ...
      <div className="mx-auto max-w-5xl">
        ...
        {createdPo && (
          <p className="mb-4 rounded border border-emerald-800 bg-emerald-950 px-3 py-2 text-sm text-emerald-300">
            Purchase order created — <span className="font-semibold">{createdPo.poReference}</span>.
          </p>
        )}
        ...
        {isLoading ? (
          ...
          <form onSubmit={handleCreate} className="space-y-6 rounded-lg border border-slate-800 bg-slate-900 p-6">
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
              <div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    ...
                    className="flex-none rounded border border-slate-600 px-3 py-2 text-xs text-slate-300 hover:border-emerald-500 hover:text-emerald-400"
                  >
                    +
                  </button>
                </div>
                {showNewSupplier && (
                  <div className="mt-2 flex gap-2 rounded border border-slate-700 bg-slate-800 p-3">
                    <input
                    ...
                    <input
                      ...
                      className="w-32 rounded border border-slate-700 bg-slate-900 px-3 py-1.5 text-sm"
                    />
                    <button
                      type="button"
              ...
              <div>
                <label className="mb-1 block text-xs text-slate-400">PO Number</label>
                <input
                  value="Assigned automatically on save"
                  readOnly
                  className="w-full cursor-not-allowed rounded border border-slate-700 bg-slate-800 px-3 py-2 text-sm text-slate-500"
                />
              </div>

              <div>
                <label className="mb-1 block text-xs text-slate-400">PO Date</label>
                <input
                  value={poDatePreview}
                  readOnly
                  className="w-full cursor-not-allowed rounded border border-slate-700 bg-slate-800 px-3 py-2 text-sm text-slate-400"
                />
              </div>
            </div>
