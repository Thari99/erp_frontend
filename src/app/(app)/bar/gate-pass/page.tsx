...
import { api, ApiError, type BarGatePass, type BarPurchaseOrder } from '@/lib/api';
...
export default function ManageGatePassPage() {
  return (
    <Suspense fallback={null}>
      <ManageGatePassPageContent />
    </Suspense>
  );
}

function ManageGatePassPageContent() {
  ...
  const [openOrders, setOpenOrders] = useState<BarPurchaseOrder[]>([]);
  const [gatePasses, setGatePasses] = useState<BarGatePass[]>([]);
  ...
  const load = useCallback(async () => {
    try {
      const [orders, passes] = await Promise.all([api.listOpenBarPurchaseOrders(), api.listBarGatePasses()]);
  ...
  // Coming here from a PO's "Gate Pass" action: jump straight to receiving it if it's
  // still open, or scroll to, page to, and highlight its receipt if it's already been received.
  useEffect(() => {
  ...
  function startReceive(order: BarPurchaseOrder) {
    setReceivingId(order.id);
    const defaults: Record<string, string> = {};
    for (const item of order.items) {
      defaults[item.productId] = String(item.orderedQty);
    }
  ...
  function damageQtyFor(order: BarPurchaseOrder, productId: string) {
    const item = order.items.find((i) => i.productId === productId);
    const ordered = item?.orderedQty ?? 0;
    const received = Number(receiveQty[productId] ?? 0);
    return Math.max(ordered - received, 0);
  }

  async function submitReceive(order: BarPurchaseOrder) {
    const items = order.items
      .map((item) => ({
        productId: item.productId,
        receivedQty: Number(receiveQty[item.productId] ?? 0),
        damageQty: damageQtyFor(order, item.productId),
        expiryDate: receiveExpiry[item.productId] || undefined,
        remark: receiveRemark[item.productId]?.trim() || undefined,
      }))
      .filter((item) => item.receivedQty >= 0);
    setIsReceiving(true);
    setError(null);
    try {
      await api.receiveBarGatePass({ purchaseOrderId: order.id, vehicleNo: vehicleNo.trim() || undefined, items });
      setReceivingId(null);
      await load();
    } catch (err) {
    ...
    <main className="px-6 py-10 text-slate-50">
      <div className="mx-auto max-w-6xl">
        <header className="mb-8">
          <p className="text-sm text-emerald-400">Bar</p>
                        ...
                        <div key={item.id} className="rounded border border-slate-800 bg-slate-950/60 p-3">
                          <div className="flex flex-wrap items-center justify-between gap-2 text-sm">
                            <span className="text-slate-300">
                              {item.product.name} <span className="text-slate-500">(ordered {item.orderedQty})</span>
                            </span>
                            <div className="flex items-center gap-2">
                              ...
                              <input
                                ...
                                onChange={(e) => setReceiveQty((prev) => ({ ...prev, [item.productId]: e.target.value }))}
                              ...
                              <input
                                type="text"
                                readOnly
                                value={damageQtyFor(order, item.productId)}
                          ...
                          <div className="mt-2 flex flex-wrap items-center gap-2 text-sm">
                            ...
                            <input
                              type="date"
                              value={receiveExpiry[item.productId] ?? ''}
                              onChange={(e) => setReceiveExpiry((prev) => ({ ...prev, [item.productId]: e.target.value }))}
                            ...
                            <input
                              type="text"
                              value={receiveRemark[item.productId] ?? ''}
                              onChange={(e) => setReceiveRemark((prev) => ({ ...prev, [item.productId]: e.target.value }))}
                              placeholder="Ex: 2 bottles cracked"
                        ...
                        <div className="inline-block text-left" ref={openMenuId === pass.id ? menuRef : undefined}>
                          ...
                          {openMenuId === pass.id && menuPosition && (
                            ...
                            >
                              ...
                              <Link
                                href={`/bar/gate-pass/${pass.id}?autoprint=1`}
                                onClick={() => setOpenMenuId(null)}
                                className="block px-3 py-1.5 text-xs text-slate-200 hover:bg-slate-700"
                              >
                                Print
