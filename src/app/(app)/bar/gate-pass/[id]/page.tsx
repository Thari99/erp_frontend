...
import { api, ApiError, type BarGatePass } from '@/lib/api';
...
export default function GatePassDetailPage() {
  return (
    <Suspense fallback={null}>
      <GatePassDetailPageContent />
    </Suspense>
  );
}

function GatePassDetailPageContent() {
  ...
  const [gatePass, setGatePass] = useState<BarGatePass | null>(null);
  ...
  const orderedQtyByProduct = new Map(gatePass.purchaseOrder.items.map((item) => [item.productId, item.orderedQty]));
              ...
              <tbody>
                {gatePass.items.map((item) => (
                  <tr key={item.id} className="border-t border-slate-800 print:border-black">
                    ...
                    <td className="px-3 py-2">{orderedQtyByProduct.get(item.productId) ?? '—'}</td>
