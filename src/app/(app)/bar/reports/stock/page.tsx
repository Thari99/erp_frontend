...
import { api, ApiError, type BarStockReport } from '@/lib/api';
import { useRequireLogin } from '@/lib/use-require-login';

export default function BarStockReportPage() {
  const requireLogin = useRequireLogin();
  const [report, setReport] = useState<BarStockReport | null>(null);
  const [isLoading, setIsLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  const load = useCallback(async () => {
    try {
      setReport(await api.barStockReport());
    } catch (err) {
                ...
                <tbody>
                  {report.rows.map((row) => (
                    <tr key={row.id} className="border-t border-slate-800">
                      ...
                      <td className="px-2 py-2 text-slate-400">{row.brand ?? '—'}</td>
