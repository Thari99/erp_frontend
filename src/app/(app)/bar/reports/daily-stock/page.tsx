...
import { api, ApiError, type BarDailyStockReport } from '@/lib/api';
...
export default function BarDailyStockReportPage() {
  const requireLogin = useRequireLogin();
  const [date, setDate] = useState(today());
  const [report, setReport] = useState<BarDailyStockReport | null>(null);
  ...
  const load = useCallback(async () => {
    setIsLoading(true);
    try {
      setReport(await api.barDailyStockReport(date));
    } catch (err) {
      ...
      <div className="mx-auto max-w-4xl">
        <header className="mb-8 flex flex-wrap items-end justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">Opening, received, sold/adjusted, and closing balance per product for one day.</p>
          </div>
        ...
        <label className="mb-3 flex items-center gap-2 text-sm text-slate-400">
          ...
          Only show products with movement that day
        </label>
          ...
          <table className="w-full text-left text-sm">
            ...
            <tbody>
              {isLoading ? (
                ...
                <tr>
                  <td colSpan={7} className="px-4 py-6 text-center text-slate-500">
                    {onlyActivity ? 'No stock movement on this day.' : 'No products found.'}
                  </td>
                </tr>
              ) : (
                rows.map((row) => (
