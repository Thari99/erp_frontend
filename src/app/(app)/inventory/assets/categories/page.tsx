...
import { api, ApiError, type PhysicalAssetCategory } from '@/lib/api';
import { useRequireLogin } from '@/lib/use-require-login';

export default function ManageAssetCategoryPage() {
  ...
  function startEdit(category: PhysicalAssetCategory) {
  ...
  async function submitEdit(event: React.FormEvent) {
    ...
    setError(null);
    try {
      await api.updateAssetCategory(editingId, { name: editName.trim() });
      setEditingId(null);
      await load();
    } catch (err) {
  ...
  async function toggleActive(category: PhysicalAssetCategory) {
    setBusyId(category.id);
    setError(null);
    try {
      await api.updateAssetCategory(category.id, { isActive: !category.isActive });
      await load();
    } catch (err) {
      if (!requireLogin(err)) setError(err instanceof ApiError ? err.message : 'Failed to update category');
    } finally {
  ...
  async function handleDelete(category: PhysicalAssetCategory) {
    if (!window.confirm(`Delete "${category.name}"?`)) return;
    setBusyId(category.id);
    setError(null);
    try {
      await api.deleteAssetCategory(category.id);
      await load();
    } catch (err) {
      ...
      <div className="mx-auto max-w-3xl">
        <header className="mb-8 flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <h1 className="text-2xl font-semibold">Manage Asset Category</h1>
          </div>
          <Link
            href="/inventory/assets/categories/new"
            ...
            <tbody>
              {isLoading ? (
                ...
                <tr>
                  <td colSpan={3} className="px-4 py-6 text-center text-slate-500">
                    No categories yet —{' '}
                    <Link href="/inventory/assets/categories/new" className="text-emerald-400 underline">
                      create one
                    </Link>
                    .
                  </td>
