...
import { api, ApiError, type InventoryVendor } from '@/lib/api';
import { useRequireLogin } from '@/lib/use-require-login';

export default function ManageInventorySuppliersPage() {
  ...
  const load = useCallback(async () => {
    try {
      setVendors(await api.listInventoryVendors());
    } catch (err) {
  ...
  function startEdit(vendor: InventoryVendor) {
  ...
  async function submitEdit(event: React.FormEvent) {
    ...
    setError(null);
    try {
      await api.updateInventoryVendor(editingId, {
        name: editName.trim(),
  ...
  async function toggleActive(vendor: InventoryVendor) {
    setBusyId(vendor.id);
    setError(null);
    try {
      await api.updateInventoryVendor(vendor.id, { isActive: !vendor.isActive });
      await load();
    } catch (err) {
      if (!requireLogin(err)) setError(err instanceof ApiError ? err.message : 'Failed to update supplier');
    } finally {
  ...
  async function handleDelete(vendor: InventoryVendor) {
    if (!window.confirm(`Delete "${vendor.name}"?`)) return;
    setBusyId(vendor.id);
    setError(null);
    try {
      await api.deleteInventoryVendor(vendor.id);
      await load();
    } catch (err) {
            ...
            <tbody>
              {isLoading ? (
                ...
                <tr>
                  <td colSpan={5} className="px-4 py-6 text-center text-slate-500">
                    No suppliers yet —{' '}
                    <Link href="/inventory/vendors/new" className="text-emerald-400 underline">
                      create one
                    </Link>
                    .
                  </td>
