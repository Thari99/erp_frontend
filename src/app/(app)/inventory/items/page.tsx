...
import { api, ApiError, type InventoryItem, type InventoryStockMovement } from '@/lib/api';
...
export default function ManageInventoryItemPage() {
  ...
  async function submitAdjust(item: InventoryItem) {
    ...
    setError(null);
    try {
      await api.adjustInventoryItemStock(item.id, { quantity, reason: adjustReason || undefined });
      setAdjustingId(null);
      await load();
    } catch (err) {
  ...
  async function toggleHistory(itemId: string) {
    ...
    try {
      setHistory(await api.listInventoryItemStockMovements(itemId));
    } catch (err) {
      ...
      <div className="w-full">
        <header className="mb-8 flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <h1 className="text-2xl font-semibold">Manage Item</h1>
            <p className="mt-1 text-sm text-slate-500">
              Manage categories under <Link href="/inventory/categories" className="text-emerald-400 underline">Category</Link>.
            </p>
          </div>
          <Link
        ...
        <section className="rounded-lg border border-slate-800 bg-slate-900 p-6">
          <h2 className="mb-4 text-sm font-medium uppercase tracking-wide text-slate-400">Items ({items.length})</h2>
          {isLoading ? (
            <p className="text-sm text-slate-500">Loading…</p>
          ) : items.length === 0 ? (
            <p className="text-sm text-slate-500">
              No items yet —{' '}
              <Link href="/inventory/items/new" className="text-emerald-400 underline">
                create one
              </Link>
              .
            </p>
