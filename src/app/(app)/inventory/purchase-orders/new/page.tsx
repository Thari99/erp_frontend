...
import { api, ApiError, type InventoryItem, type InventoryPurchaseOrder, type InventoryVendor } from '@/lib/api';
...
export default function CreateInventoryPurchaseOrderPage() {
  const requireLogin = useRequireLogin();
  const [suppliers, setSuppliers] = useState<InventoryVendor[]>([]);
  const [items, setItems] = useState<InventoryItem[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [createdPo, setCreatedPo] = useState<InventoryPurchaseOrder | null>(null);
  ...
  const load = useCallback(async () => {
    try {
      const [supplierList, itemList] = await Promise.all([api.listInventoryVendors(), api.listInventoryItems()]);
  ...
  async function handleAddSupplier(event: React.FormEvent) {
    ...
    try {
      const supplier = await api.createInventoryVendor({ name: newSupplierName, phone: newSupplierPhone || undefined });
  ...
  async function handleCreate(event: React.FormEvent) {
    ...
    setIsSaving(true);
    try {
      const po = await api.createInventoryPurchaseOrder({
        vendorId: supplierId,
