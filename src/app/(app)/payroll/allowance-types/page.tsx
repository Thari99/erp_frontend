...
import { api, ApiError, type PayrollAllowanceType } from '@/lib/api';
import { useRequireLogin } from '@/lib/use-require-login';

export default function ManageAllowanceTypePage() {
  const requireLogin = useRequireLogin();
  const [types, setTypes] = useState<PayrollAllowanceType[]>([]);
  ...
  const load = useCallback(async () => {
    try {
      setTypes(await api.listPayrollAllowanceTypes());
    } catch (err) {
      if (!requireLogin(err)) setError(err instanceof ApiError ? err.message : 'Failed to load allowance types');
    } finally {
      setIsLoading(false);
    }
  }, [requireLogin]);
  ...
  async function toggleActive(type: PayrollAllowanceType) {
    setBusyId(type.id);
    setError(null);
    try {
      await api.updatePayrollAllowanceType(type.id, { isActive: !type.isActive });
      await load();
    } catch (err) {
      if (!requireLogin(err)) setError(err instanceof ApiError ? err.message : 'Failed to update allowance type');
    } finally {
      setBusyId(null);
    }
  }

  async function handleDelete(type: PayrollAllowanceType) {
    if (!window.confirm(`Delete "${type.name}"?`)) return;
    setBusyId(type.id);
    setError(null);
    try {
      await api.deletePayrollAllowanceType(type.id);
      await load();
    } catch (err) {
      if (!requireLogin(err)) setError(err instanceof ApiError ? err.message : 'Failed to delete allowance type');
    } finally {
      setBusyId(null);
    }
  }
      ...
      <div className="mx-auto max-w-3xl">
        <header className="mb-8 flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <h1 className="text-2xl font-semibold">Manage Allowance Type</h1>
          </div>
          <Link
            href="/payroll/allowance-types/new"
              ...
              {isLoading ? (
                ...
                <tr>
                  <td colSpan={4} className="px-4 py-6 text-center text-slate-500">
                    ...
                    <Link href="/payroll/allowance-types/new" className="text-emerald-400 underline">
                      create one
                    </Link>
                    .
                  </td>
