...
import { api, ApiError, type FoodPurchaseOrder, type FoodStockItem, type FoodVendor } from '@/lib/api';
...
export default function CreateFoodPurchaseOrderPage() {
  ...
  const [createdPo, setCreatedPo] = useState<FoodPurchaseOrder | null>(null);
  ...
  const load = useCallback(async () => {
    try {
      const [supplierList, stockItemList] = await Promise.all([api.listFoodVendors(), api.listFoodStockItems()]);
  ...
  async function handleAddSupplier(event: React.FormEvent) {
    ...
    try {
      const supplier = await api.createFoodVendor({ name: newSupplierName, phone: newSupplierPhone || undefined });
  ...
  async function handleCreate(event: React.FormEvent) {
    ...
    setIsSaving(true);
    try {
      const po = await api.createFoodPurchaseOrder({
        vendorId: supplierId,
          ...
          <form onSubmit={handleCreate} className="space-y-6 rounded-lg border border-slate-800 bg-slate-900 p-6">
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
              ...
              <div>
                ...
                <input value={poDatePreview} readOnly className="w-full cursor-not-allowed rounded border border-slate-700 bg-slate-800 px-3 py-2 text-sm text-slate-400" />
              </div>
            </div>
