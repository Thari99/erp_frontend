...
import { api, ApiError, type FoodPurchaseOrder } from '@/lib/api';
...
const STATUS_DOT: Record<FoodPurchaseOrder['status'], string> = {
  OPEN: 'bg-amber-500',
  RECEIVED: 'bg-emerald-500',
  CANCELLED: 'bg-red-500',
};

export default function ManageFoodPurchaseOrdersPage() {
  const requireLogin = useRequireLogin();
  const [orders, setOrders] = useState<FoodPurchaseOrder[]>([]);
  ...
  function toggleMenu(order: FoodPurchaseOrder, event: React.MouseEvent<HTMLButtonElement>) {
  ...
  const load = useCallback(async () => {
    try {
      setOrders(await api.listFoodPurchaseOrders());
    } catch (err) {
  ...
  function refLabel(order: FoodPurchaseOrder) {
    return order.poReference ?? `PO-${order.poNo}`;
  }

  async function handleCancel(order: FoodPurchaseOrder) {
    setOpenMenuId(null);
    if (!window.confirm(`Cancel ${refLabel(order)}?`)) return;
    setBusyId(order.id);
    setError(null);
    try {
      await api.cancelFoodPurchaseOrder(order.id);
      await load();
    } catch (err) {
  ...
  async function handleDelete(order: FoodPurchaseOrder) {
    setOpenMenuId(null);
    if (!window.confirm(`Delete ${refLabel(order)}? This can't be undone.`)) return;
    setBusyId(order.id);
    setError(null);
    try {
      await api.deleteFoodPurchaseOrder(order.id);
      await load();
    } catch (err) {
    ...
    <main className="px-6 py-10 text-slate-50">
      <div className="mx-auto max-w-4xl">
        <header className="mb-8 flex flex-wrap items-start justify-between gap-3">
          <div>
            <p className="text-sm text-emerald-400">Food · Purchase Orders</p>
            <h1 className="text-2xl font-semibold">Manage Purchase Orders</h1>
            <p className="mt-1 text-sm text-slate-500">
              Manage suppliers under{' '}
              <Link href="/food/vendors" className="text-emerald-400 underline">
                Suppliers
              </Link>
              .
            </p>
          </div>
          <Link href="/food/purchase-orders/new" className="rounded bg-emerald-600 px-4 py-2 text-sm font-medium text-white hover:bg-emerald-500">
                ...
                orders.map((order) => (
                  <Fragment key={order.id}>
                    <tr className="border-t border-slate-800 text-slate-200 hover:bg-slate-800/60">
                      <td className="px-4 py-3 font-mono text-xs text-slate-400">{order.poReference ?? `PO-${String(order.poNo).padStart(5, '0')}`}</td>
                        ...
                        <div className="inline-block text-left" ref={openMenuId === order.id ? menuRef : undefined}>
                          ...
                          {openMenuId === order.id && menuPosition && (
                            ...
                            >
                              {order.status === 'OPEN' && (
                                <Link
                                  href={`/food/purchase-orders/${order.id}/edit`}
                              ...
                              <Link
                                href={`/food/purchase-orders/${order.id}`}
                                onClick={() => setOpenMenuId(null)}
                                className="block px-3 py-1.5 text-xs text-slate-200 hover:bg-slate-700"
                              >
                                Read more
                              ...
                              {order.status !== 'CANCELLED' && (
                                <Link
                                  href={`/food/gate-pass?po=${order.id}`}
                                  onClick={() => setOpenMenuId(null)}
                                  className="block px-3 py-1.5 text-xs text-slate-200 hover:bg-slate-700"
                                >
                                  Gate Pass
                              ...
                              <Link
                                href={`/food/purchase-orders/${order.id}?autoprint=1`}
                                onClick={() => setOpenMenuId(null)}
                                className="block px-3 py-1.5 text-xs text-slate-200 hover:bg-slate-700"
                              >
                                Print
                              ...
                              {order.status === 'OPEN' && (
                                <button type="button" onClick={() => handleCancel(order)} className="block w-full px-3 py-1.5 text-left text-xs text-amber-400 hover:bg-slate-700">
                                  Cancel
                                </button>
                              )}
                              {order.status !== 'RECEIVED' && (
                                <button type="button" onClick={() => handleDelete(order)} className="block w-full px-3 py-1.5 text-left text-xs text-red-400 hover:bg-slate-700">
                                  Delete
                                </button>
                              )}
                            </div>
                          ...
                          <ul className="space-y-1 text-xs text-slate-400">
                            {order.items.map((item) => (
                              <li key={item.id} className="flex justify-between">
                                <span>
                                  {item.orderedQty} × {item.stockItem.name} @ Rs. {Number(item.unitCost).toFixed(2)}
                                </span>
