...
export default function FoodPurchaseOrderDetailPage() {
  return (
    <Suspense fallback={null}>
      <FoodPurchaseOrderDetailPageContent />
    </Suspense>
  );
}

function FoodPurchaseOrderDetailPageContent() {
  ...
  const [po, setPo] = useState<FoodPurchaseOrder | null>(null);
  ...
  useEffect(() => {
    if (!params.id) return;
    api
      .getFoodPurchaseOrder(params.id)
      .then(setPo)
      .catch((err) => {
      ...
      <div className="mx-auto max-w-3xl">
        <div className="mb-6 flex flex-wrap items-center justify-between gap-3 print:hidden">
          ...
          <div className="flex gap-2">
            {po.status === 'OPEN' && (
              <Link
                href={`/food/purchase-orders/${po.id}/edit`}
