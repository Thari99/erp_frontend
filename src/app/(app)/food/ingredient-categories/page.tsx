...
import { api, ApiError, type FoodIngredientCategory } from '@/lib/api';
import { useRequireLogin } from '@/lib/use-require-login';

export default function ManageIngredientCategoryPage() {
  ...
  const load = useCallback(async () => {
    try {
      setCategories(await api.listFoodIngredientCategories());
    } catch (err) {
  ...
  function startEdit(category: FoodIngredientCategory) {
  ...
  async function submitEdit(event: React.FormEvent) {
    ...
    setError(null);
    try {
      await api.updateFoodIngredientCategory(editingId, { name: editName.trim() });
      setEditingId(null);
      await load();
    } catch (err) {
  ...
  async function toggleActive(category: FoodIngredientCategory) {
    setBusyId(category.id);
    setError(null);
    try {
      await api.updateFoodIngredientCategory(category.id, { isActive: !category.isActive });
      await load();
    } catch (err) {
      if (!requireLogin(err)) setError(err instanceof ApiError ? err.message : 'Failed to update category');
    } finally {
  ...
  async function handleDelete(category: FoodIngredientCategory) {
    if (!window.confirm(`Delete "${category.name}"?`)) return;
    setBusyId(category.id);
    setError(null);
    try {
      await api.deleteFoodIngredientCategory(category.id);
      await load();
    } catch (err) {
    ...
    <main className="px-6 py-10 text-slate-50">
      <div className="mx-auto max-w-3xl">
        <header className="mb-8 flex flex-wrap items-start justify-between gap-3">
          <div>
            <p className="text-sm text-emerald-400">Food · Ingredient Category</p>
            <h1 className="text-2xl font-semibold">Manage Ingredient Category</h1>
          </div>
          <Link
            href="/food/ingredient-categories/new"
            ...
            <tbody>
              {isLoading ? (
                ...
                <tr>
                  <td colSpan={3} className="px-4 py-6 text-center text-slate-500">
                    No categories yet —{' '}
                    <Link href="/food/ingredient-categories/new" className="text-emerald-400 underline">
                      create one
                    </Link>
                    .
                  </td>
