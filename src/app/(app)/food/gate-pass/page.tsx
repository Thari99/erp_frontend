...
import { api, ApiError, type FoodGatePass, type FoodPurchaseOrder } from '@/lib/api';
...
export default function ManageFoodGatePassPage() {
  return (
    <Suspense fallback={null}>
      <ManageFoodGatePassPageContent />
    </Suspense>
  );
}

function ManageFoodGatePassPageContent() {
  ...
  const [openOrders, setOpenOrders] = useState<FoodPurchaseOrder[]>([]);
  const [gatePasses, setGatePasses] = useState<FoodGatePass[]>([]);
  ...
  const load = useCallback(async () => {
    try {
      const [orders, passes] = await Promise.all([api.listOpenFoodPurchaseOrders(), api.listFoodGatePasses()]);
  ...
  function startReceive(order: FoodPurchaseOrder) {
    setReceivingId(order.id);
    const defaults: Record<string, string> = {};
    for (const item of order.items) {
      defaults[item.stockItemId] = String(item.orderedQty);
    }
  ...
  function damageQtyFor(order: FoodPurchaseOrder, stockItemId: string) {
    const item = order.items.find((i) => i.stockItemId === stockItemId);
    const ordered = item?.orderedQty ?? 0;
    const received = Number(receiveQty[stockItemId] ?? 0);
    return Math.max(ordered - received, 0);
  }

  async function submitReceive(order: FoodPurchaseOrder) {
    const items = order.items
      .map((item) => ({
        stockItemId: item.stockItemId,
        receivedQty: Number(receiveQty[item.stockItemId] ?? 0),
        damageQty: damageQtyFor(order, item.stockItemId),
        expiryDate: receiveExpiry[item.stockItemId] || undefined,
        remark: receiveRemark[item.stockItemId]?.trim() || undefined,
      }))
      .filter((item) => item.receivedQty >= 0);
    setIsReceiving(true);
    setError(null);
    try {
      await api.receiveFoodGatePass({ purchaseOrderId: order.id, vehicleNo: vehicleNo.trim() || undefined, items });
      setReceivingId(null);
      await load();
    } catch (err) {
    ...
    <main className="px-6 py-10 text-slate-50">
      <div className="mx-auto max-w-6xl">
        <header className="mb-8">
          <p className="text-sm text-emerald-400">Food</p>
          <h1 className="text-2xl font-semibold">Manage Gate Pass</h1>
          <p className="mt-1 text-sm text-slate-500">Receive ingredient purchase orders — stock updates automatically.</p>
        </header>
                  ...
                  <div className="flex flex-wrap items-start justify-between gap-2">
                    <div>
                      ...
                      <p className="text-xs text-slate-500">
                        Rs. {Number(order.totalAmount).toFixed(2)} · {order.items.length} item(s)
                      </p>
                    </div>
                        ...
                        <div key={item.id} className="rounded border border-slate-800 bg-slate-950/60 p-3">
                          <div className="flex flex-wrap items-center justify-between gap-2 text-sm">
                            <span className="text-slate-300">
                              {item.stockItem.name} <span className="text-slate-500">(ordered {item.orderedQty})</span>
                            </span>
                            <div className="flex items-center gap-2">
                              <label className="text-xs text-slate-500">Received</label>
                              <input
                                type="number"
                                value={receiveQty[item.stockItemId] ?? ''}
                                onChange={(e) => setReceiveQty((prev) => ({ ...prev, [item.stockItemId]: e.target.value }))}
                              ...
                              <input
                                type="text"
                                readOnly
                                value={damageQtyFor(order, item.stockItemId)}
                          ...
                          <div className="mt-2 flex flex-wrap items-center gap-2 text-sm">
                            ...
                            <input
                              type="date"
                              value={receiveExpiry[item.stockItemId] ?? ''}
                              onChange={(e) => setReceiveExpiry((prev) => ({ ...prev, [item.stockItemId]: e.target.value }))}
                              className="rounded border border-slate-700 bg-slate-800 px-2 py-1 text-sm"
                            />
                            <label className="ml-2 text-xs text-slate-500">Condition/Remark</label>
                            <input
                              type="text"
                              value={receiveRemark[item.stockItemId] ?? ''}
                              onChange={(e) => setReceiveRemark((prev) => ({ ...prev, [item.stockItemId]: e.target.value }))}
                              placeholder="Ex: 2 packets damaged"
        ...
        <section className="rounded-lg border border-slate-800 bg-slate-900 p-6">
                  ...
                  pagedPasses.map((pass) => (
                    ...
                    >
                      <td className="whitespace-nowrap px-3 py-3 font-mono text-xs text-slate-300">
                        {pass.gatePassReference ?? `GPF-${String(pass.gatePassNo).padStart(5, '0')}`}
                      </td>
                      ...
                      <td className="whitespace-nowrap px-3 py-3">
                        <span className="inline-flex items-center rounded-full bg-emerald-500/15 px-3 py-1 text-xs font-medium text-emerald-400">Received</span>
                      </td>
                      <td className="whitespace-nowrap px-3 py-3">
                        <div className="inline-block text-left" ref={openMenuId === pass.id ? menuRef : undefined}>
                          ...
                          {openMenuId === pass.id && menuPosition && (
                            ...
                            >
                              <Link href={`/food/gate-pass/${pass.id}`} onClick={() => setOpenMenuId(null)} className="block px-3 py-1.5 text-xs text-slate-200 hover:bg-slate-700">
                                Read more
                              </Link>
                              <Link
                                href={`/food/gate-pass/${pass.id}?autoprint=1`}
          ...
          {!isLoading && filteredPasses.length > 0 && (
            <div className="mt-3 flex flex-wrap items-center justify-between gap-3 text-xs text-slate-500">
              <p>
                Showing {(currentPage - 1) * PAGE_SIZE + 1} to {Math.min(currentPage * PAGE_SIZE, filteredPasses.length)} of {filteredPasses.length} results
              </p>
              {totalPages > 1 && (
