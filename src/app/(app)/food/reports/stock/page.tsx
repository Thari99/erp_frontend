...
import { api, ApiError, type FoodStockReport } from '@/lib/api';
import { useRequireLogin } from '@/lib/use-require-login';

export default function FoodStockReportPage() {
  const requireLogin = useRequireLogin();
  const [report, setReport] = useState<FoodStockReport | null>(null);
  const [isLoading, setIsLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  const load = useCallback(async () => {
    try {
      setReport(await api.foodStockReport());
    } catch (err) {
    ...
    <main className="px-6 py-10 text-slate-50">
      <div className="mx-auto max-w-3xl">
        <header className="mb-8">
          ...
          <p className="mt-1 text-sm text-slate-500">Ingredient stock levels — menu items themselves aren&apos;t stock-tracked.</p>
        </header>
        ...
        {isLoading ? (
          ...
          <>
            <div className="mb-6 grid grid-cols-1 gap-3 sm:grid-cols-2">
              ...
              <div className="rounded-lg border border-slate-800 bg-slate-900 p-4">
                <p className="text-xs uppercase tracking-wide text-slate-500">Low Stock Ingredients</p>
