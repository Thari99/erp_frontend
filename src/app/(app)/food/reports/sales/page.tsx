...
import { api, ApiError, type FoodSalesReport } from '@/lib/api';
...
export default function FoodSalesReportPage() {
  ...
  const [report, setReport] = useState<FoodSalesReport | null>(null);
  ...
  const load = useCallback(async () => {
    setIsLoading(true);
    try {
      setReport(await api.foodSalesReport(from ? `${from}T00:00:00` : undefined, to ? `${to}T23:59:59` : undefined));
    } catch (err) {
      ...
      <div className="mx-auto max-w-3xl">
        ...
        <div className="mb-6 flex flex-wrap items-end gap-3 rounded-lg border border-slate-800 bg-slate-900 p-4">
          <div>
            ...
            <input type="date" value={from} onChange={(e) => setFrom(e.target.value)} className="rounded border border-slate-700 bg-slate-800 px-3 py-2 text-sm" />
          </div>
          <div>
            <p className="mb-1 text-xs text-slate-400">To</p>
            <input type="date" value={to} onChange={(e) => setTo(e.target.value)} className="rounded border border-slate-700 bg-slate-800 px-3 py-2 text-sm" />
          </div>
        </div>
        ...
        {isLoading ? (
          ...
          <>
            ...
            <section className="rounded-lg border border-slate-800 bg-slate-900 p-6">
              <h2 className="mb-4 text-sm font-medium uppercase tracking-wide text-slate-400">Top Menu Items</h2>
                ...
                <table className="w-full text-left text-sm">
                  ...
                  <tbody>
                    {report.topItems.map((item) => (
                      <tr key={item.foodItemId} className="border-t border-slate-800">
                        <td className="px-2 py-2 text-slate-200">{item.name}</td>
                        <td className="px-2 py-2 text-slate-300">{item.quantity}</td>
                        <td className="px-2 py-2 text-slate-300">Rs. {item.revenue.toFixed(2)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
