...
export default function FoodSaleReceiptPage() {
  return (
    <Suspense fallback={null}>
      <FoodSaleReceiptPageContent />
    </Suspense>
  );
}

function FoodSaleReceiptPageContent() {
  ...
  const [sale, setSale] = useState<FoodSale | null>(null);
      ...
      <div className="mx-auto max-w-sm">
        ...
        <div className="rounded-lg border border-slate-800 bg-slate-900 p-6 print:border-black print:bg-white">
          <div className="mb-4 text-center">
            <p className="text-sm text-emerald-400 print:text-black">Food Receipt</p>
          ...
          <div className="mb-4 space-y-1 text-xs text-slate-400 print:text-slate-700">
            ...
            <p>Order: {sale.orderType === 'DINE_IN' ? `Dine In${sale.table ? ` — ${sale.table.name}` : ''}` : 'Takeaway'}</p>
          ...
          {sale.remarks && <p className="mt-3 border-t border-slate-800 pt-3 text-xs text-slate-500 print:border-black">Remarks: {sale.remarks}</p>}
        </div>
      </div>
    </main>
  );
