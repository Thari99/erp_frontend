...
export default function CreateFoodVendorPage() {
  ...
  async function handleCreate(event: React.FormEvent) {
    ...
    setIsSaving(true);
    try {
      await api.createFoodVendor({ name, phone: phone || undefined, address: address || undefined, isActive: status === 'ACTIVE' });
      setName('');
      setPhone('');
      setAddress('');
      setStatus('ACTIVE');
  ...
  return (
    <main className="px-6 py-10 text-slate-50">
      <div className="mx-auto max-w-md">
        ...
        {saved && <p className="mb-4 rounded border border-emerald-800 bg-emerald-950 px-3 py-2 text-sm text-emerald-300">Supplier added.</p>}
        ...
        <form onSubmit={handleCreate} className="space-y-4 rounded-lg border border-slate-800 bg-slate-900 p-6">
          <div>
            ...
            <input
              placeholder="e.g. Fresh Produce Traders"
              value={name}
          ...
          <div>
            ...
            <textarea value={address} onChange={(e) => setAddress(e.target.value)} rows={2} className="w-full rounded border border-slate-700 bg-slate-800 px-3 py-2 text-sm" />
          </div>
