...
export default function ManageFoodCategoryPage() {
  ...
  function startEdit(category: FoodCategory) {
  ...
  async function submitEdit(event: React.FormEvent) {
    ...
    setError(null);
    try {
      await api.updateFoodCategory(editingId, { name: editName.trim() });
      setEditingId(null);
      await load();
    } catch (err) {
  ...
  async function toggleActive(category: FoodCategory) {
    ...
    setError(null);
    try {
      await api.updateFoodCategory(category.id, { isActive: !category.isActive });
      await load();
    } catch (err) {
      if (!requireLogin(err)) setError(err instanceof ApiError ? err.message : 'Failed to update category');
    } finally {
  ...
  async function handleDelete(category: FoodCategory) {
    ...
    setError(null);
    try {
      await api.deleteFoodCategory(category.id);
      await load();
    } catch (err) {
    ...
    <main className="px-6 py-10 text-slate-50">
      <div className="mx-auto max-w-3xl">
        <header className="mb-8 flex flex-wrap items-start justify-between gap-3">
          <div>
            <p className="text-sm text-emerald-400">Food · Category</p>
            <h1 className="text-2xl font-semibold">Manage Category</h1>
          </div>
          <Link href="/food/categories/new" className="rounded bg-emerald-600 px-4 py-2 text-sm font-medium text-white hover:bg-emerald-500">
            + Create Category
          </Link>
        </header>
            ...
            <tbody>
              {isLoading ? (
                ...
                <tr>
                  <td colSpan={3} className="px-4 py-6 text-center text-slate-500">
                    No categories yet —{' '}
                    <Link href="/food/categories/new" className="text-emerald-400 underline">
                      create one
                    </Link>
                    .
                  </td>
